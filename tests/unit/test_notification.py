...
class TestNotification(TestCase):
    ...
    def test_convert_to_cuf_format_when_launched_at_before_audit_period(self):
        ...
        expected_cuf_xml = utils.generate_verified_message_in_cuf_format(values,
            'some_other_uuid', 'some_uuid', 'compute.instance.exists.verified')

        notification = Notification(exists_message)
        verified_message = notification.convert_to_verified_message_in_cuf_format(
            {'region':'DFW','data_center':'DFW1'})

        self.assertEquals(expected_cuf_xml['payload'], verified_message['payload'])
        self.assertEquals('some_other_uuid', verified_message['message_id'])
        self.assertEquals('some_uuid', verified_message['original_message_id'])
        self.assertEquals('compute.instance.exists.verified',
                          verified_message['event_type'])
